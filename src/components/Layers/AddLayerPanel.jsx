...
// Unified browse entries: admin layers + catalog layers in one list
function buildBrowseEntries() {
  const entries = []

  // Admin layers → category "Administrative Boundaries"
  for (const layer of ADMIN_LAYERS) {
    entries.push({
      id: `admin:${layer.id}`,
      name: layer.name,
      category: layer.group === 'Provinces' ? 'Provinces' : 'Regencies',
      sourceType: 'admin',
      adminLayerId: layer.id,
      provider: 'BPS / GADM',
      badge: 'Vector',
    })
  }

  // Catalog layers keep their categories
...
export default function AddLayerPanel() {
  ...
  // Load datasets when switching to data tab
  useEffect(() => {
    if (activeTab === 'data' && user && datasets.length === 0) {
  ...
  const filteredEntries = BROWSE_ENTRIES.filter((entry) => {
  ...
  // Check if a browse entry is already added
  const isEntryAdded = useCallback((entry) => {
    if (entry.sourceType === 'admin') {
      return existingLayers.some(l => l.type === 'admin' && l.adminConfig?.adminLayerId === entry.adminLayerId)
    }
  ...
  const handleAddCustomLayer = useCallback(async () => {
    ...
    if (customType === 'geojson') {
      if (!customUrl.trim()) { setCustomError('URL is required'); return }
      setLoading(true)
      const result = await addRemoteGeoJsonLayer(customDisplayName.trim() || 'Remote GeoJSON', customUrl.trim())
      setLoading(false)
      if (result?.error) { setCustomError(result.error.message) } else {
        setCustomUrl(''); setCustomDisplayName(''); switchToLayers()
      }
      return
    }
    ...
    if (customType === 'xyz') {
      if (!customUrl.includes('{z}') || !customUrl.includes('{x}') || !customUrl.includes('{y}')) {
        setCustomError('XYZ URL must contain {z}, {x}, and {y} placeholders'); return
      }
    } else if (customType === 'wms') {
      if (!customUrl.trim()) { setCustomError('URL is required'); return }
      if (!customLayerName.trim()) { setCustomError('Layer name is required for WMS'); return }
    }

    const entry = {
      id: `custom-${Date.now()}`,
      name: customDisplayName.trim() || (customType === 'wms' ? customLayerName : 'Custom Layer'),
      provider: 'Custom', category: 'Custom', type: customType,
      url: customUrl.trim(), layers: customLayerName.trim(),
      format: 'image/png', attribution: customAttribution.trim(),
      bbox: null, cors: true,
    }
    const result = addCatalogLayer(entry)
    if (result?.error) { setCustomError(result.error.message) } else {
      setCustomUrl(''); setCustomLayerName(''); setCustomDisplayName(''); setCustomAttribution(''); switchToLayers()
    }
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {TABS.map((tab) => (
          <button
            ...
            className={`flex-1 min-w-0 py-2.5 text-xs sm:text-sm font-medium transition-colors whitespace-nowrap px-2 ${
              activeTab === tab.id ? 'text-accent border-b-2 border-accent' : 'text-muted hover:text-ink'
            }`}
          >
      ...
      <div className="flex-1 overflow-y-auto pt-4">
        {/* ── Browse tab: unified admin + catalog ── */}
        {activeTab === 'browse' && (
          <div className="space-y-4">
            ...
            {filteredEntries.length === 0 && (
              <p className="text-center text-sm text-muted py-4">No layers match your search</p>
            )}
          </div>
        )}

        {/* ── Data tab: local file + cloud datasets ── */}
        {activeTab === 'data' && (
          <div className="space-y-4">
            <div>
              ...
              <label className="flex items-center justify-center gap-2 border-2 border-dashed border-border rounded-lg py-3 cursor-pointer hover:border-accent/50 hover:bg-accent/5 transition-colors">
                ...
                <input
                  ...
                  onChange={async (e) => {
                    ...
                    } catch (err) { setError('Invalid GeoJSON file: ' + err.message) }
            ...
            {!user ? (
              <p className="text-center text-sm text-muted py-4">Sign in to save datasets to the cloud</p>
            ) : datasetsLoading ? (
              ...
              <p className="text-center text-sm text-muted py-4">No cloud datasets yet.</p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted">{dataset.feature_count || '?'} features • {dataset.geometry_type}</p>
                      </div>
                      {isAdded ? (
                        <span className="text-xs text-accent px-2 py-0.5 bg-accent/10 rounded">Added</span>
                      ) : (
                        <button onClick={() => handleAddDataset(dataset)} disabled={loading}
        ...
        {activeTab === 'custom' && (
          <div className="space-y-4">
            {customError && (
              <div className="p-2 rounded bg-red-50 border border-red-200 text-xs text-red-600">{customError}</div>
            )}
            <div>
              <label className="block text-xs font-medium text-muted mb-1">Type</label>
              <select value={customType} onChange={(e) => { setCustomType(e.target.value); setCustomError(null) }}
            ...
            <div>
              ...
              <input type="text" value={customUrl} onChange={(e) => setCustomUrl(e.target.value)}
                placeholder={customType === 'xyz' ? 'https://.../{z}/{x}/{y}.png' : customType === 'geojson' ? 'https://.../data.geojson' : 'https://.../wms'}
                className="w-full rounded border border-border bg-paper px-3 py-2 text-sm placeholder:text-muted/60" />
              {customType === 'xyz' && <p className="text-xs text-muted mt-1">Must include {'{z}'}, {'{x}'}, {'{y}'}</p>}
            </div>
            {customType === 'wms' && (
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Layer Name *</label>
                <input type="text" value={customLayerName} onChange={(e) => setCustomLayerName(e.target.value)}
                  placeholder="e.g. topp:states" className="w-full rounded border border-border bg-paper px-3 py-2 text-sm placeholder:text-muted/60" />
              </div>
            )}
            <div>
              ...
              <input type="text" value={customDisplayName} onChange={(e) => setCustomDisplayName(e.target.value)}
                placeholder="My custom layer" className="w-full rounded border border-border bg-paper px-3 py-2 text-sm placeholder:text-muted/60" />
            </div>
            {customType !== 'geojson' && (
              <div>
                <label className="block text-xs font-medium text-muted mb-1">Attribution</label>
                <input type="text" value={customAttribution} onChange={(e) => setCustomAttribution(e.target.value)}
                  placeholder="© Data Provider" className="w-full rounded border border-border bg-paper px-3 py-2 text-sm placeholder:text-muted/60" />
              </div>
            )}
            <button onClick={handleAddCustomLayer}
