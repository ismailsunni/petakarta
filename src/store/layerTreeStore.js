...
const useLayerTreeStore = create(persist((set, get) => ({
  // Layer tree
  layers: [],
  ...
  addAdminLayer: (adminLayerId) => {
    ...
    if (layers.some(l => l.type === 'admin' && l.adminConfig?.adminLayerId === adminLayerId)) {
  ...
  addUserLayer: (dataset, geojson) => {
    ...
    // Check if already exists
    if (layers.some(l => l.type === 'user' && l.userConfig?.datasetId === dataset.id)) {
    ...
    set((state) => ({
      layers: state.layers.map(l =>
        l.id === layerId && l.type === 'user'
          ? { ...l, userConfig: { ...l.userConfig, style: { ...l.userConfig.style, ...styleUpdates } } }
          : l
      ),
    }))
  },
